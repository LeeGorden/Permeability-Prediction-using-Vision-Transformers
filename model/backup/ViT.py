...
def pair(t):
    return t if isinstance(t,tuple) else (t,t,t)   # isinstance() returns True if the specified object is of the specified type
...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', dim_head = 64, dropout=0., emb_dropout=0. ):
        ...
        patch_height, patch_width, patch_depth = pair(patch_size)  #patch size: the No. pixels per patch
        ...
        patch_dim = patch_height * patch_width * patch_depth
        assert pool in {'cls','mean'}, 'pool type is either cls token or mean pooling'
        
        # patch embedding
        self.to_patch_embedding = nn.Sequential(
            Rearrange('b (c p3) (h p1) (w p2) -> b (h w c) (p1 p2 p3)', p1 = patch_height, p2 = patch_width, p3 = patch_depth),
